...
pritis = ggplot(data.frame(x=0:30),aes(x)) + stat_function(fun=function(x) dgamma(x,1.2,.2),geom="area",fill="#666699")+
  labs(title=expression(paste("Prior for k"["tis"])),x=expression(paste("k"["tis"])), y = "Probability Density") +
  theme_classic() + scale_x_continuous(limits = c(0, 20), breaks = c(seq(0,20, by =5))) +
  scale_y_continuous(limits = c(0,0.75), breaks = c(seq(0,0.75, by = 0.25))) + theme(title = element_text(size = 10))

pricl = ggplot(data.frame(x=0:30),aes(x)) + stat_function(fun=function(x) dgamma(x,3.5,2.5),geom="area",fill="#666699")+
  labs(title=expression(paste("Prior for k"["class"])),x=expression(paste("k"["cl"])), y = "Probability Density") +
  theme_classic() + scale_x_continuous(limits = c(0, 20), breaks = c(seq(0,20, by =5))) +
  scale_y_continuous(limits = c(0,0.75), breaks = c(seq(0,0.75, by = 0.25))) + theme(title = element_text(size = 10))
print(pritis+pricl)
```
