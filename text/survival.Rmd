...
load("../code/survival/survmodel-ethbkd.stansave")
...
kcancplot = function(dfsamps,tisnames){
	...
	dfktis$cancname = sapply(dfktis$parname,function(x) tisnames[x])
	ggplot(dfktis) + geom_density_ridges(aes_(~value,~cancname))+ theme_minimal() + scale_fill_brewer(palette="Paired") + xlim(0,22) + labs(x="ktis multiplier",y="Cancer Type",title="Tissue Specific Effects")
}
...
kclassplot = function(dfsamps,tisnames){
	...
	dfclas$class = as.numeric(gsub("k\\[(.*),(.*)\\]","\\2",dfclas$coefname))
	dfclas$tiss = tisnames[as.numeric(gsub("k\\[(.*),(.*)\\]","\\1",dfclas$coefname))]
	ggplot(dfclas) + geom_density_ridges(aes_(~value,~class)) +
...
p1 = kcancplot(dfsamps,levels(cancerdat$type))
p2 = kclassplot(dfsamps,levels(cancerdat$type))
