...
As discussed previously, prior efforts to extract common signatures from
...
combination of both? To answer this question, we compared each cluster relative
to the background to find deferentially mutated genes. We ranked the odd ratios 
and selected the top ten enriched and depleted genes (pvalue < 0.01) for each 
cluster (**Figure 4**; odds ratios plot). We find that the clusters possess 
unique gene sets. Cluster 2 is enriched for genes responsible for protein
homeostasis. Cluster 4 is enriched for genes responsible for nucleosome 
structure. Clusters 7 and 8, which shared multiple enrichment in signaling
pathways, are largely driven by mutations in _PI3K_ and its homologs and _Ras_ 
genes, respectively (compare _PIK3CA_ and _KRAS_ panels of **Supplementary figure S5**). 
Interestingly, cluster 9 is enriched for both _PIK3CA_ and _KRAS_ which may
suggest tumors in this cluster derived from either clusters 7 or 8. Next, we 
investigated the proportion of samples per cancer type for the significant genes 
within a cluster (**Figure 4**; heatmap). Surprisingly, cluster-specific tumors 
were not predominated by one or more highly mutated genes across all cancers. 
Instead, some cancer-specific tumors in these classes had mutation profiles that
differ from one another (_e.g._ In cluster 2, LIHC was enriched for _PSME4_, KIRC
was enriched for _UBC_, and PAAD was enriched for _PSMA1_).
...
One of the limitations of knowledge-based pathway analysis is that it heavily relies on 
experimental results for the curation and validation of its pathways. The resulting 
pathways are therefore not specific to a particular tissue type but provides information
for all tissue type. Before utilizing the gene to pathway lookup table, we considered the
impact of deleterious passenger mutations for a given cancer (non-specific genes that are
not expressed) as they may introduce biases in our pathway analysis. We addressed this by
identifying genes that are lowly/not expressed in a tissue type. Using the TCGA RNA 
sequencing data adjusted for batch effect dataset (https://pancanatlas.xenahubs.net), we 
filtered the genes and tumor samples that were not included in our analysis, grouped the 
tumor samples by tissue type and computed the mean expression value for each gene. A minimum
threshold of 10 transcripts per million was set for expressed genes based on an inflection
point observed when plotting the mean expression values of genes ranked by increasing 
expression values for each tissue type. Genes that did not meet this threshold were 
considered not expressed. This operation produced a lookup table for gene expression
