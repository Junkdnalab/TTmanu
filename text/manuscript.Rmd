...
categories (\it{e.g.} “transcription factors”) or disease associations (\it{e.g.}
...
To avoid bias toward larger pathways (\it{i.e.} pathways with more member genes),
...
“silent” tissue-specific effectors (\it{i.e.} too few samples to detect above
...
for one cluster are depleted from others (\it{i.e.} is enriched in cluster 6,
...
is heterogeneous across tumors by tissue origin (\it{e.g.} In cluster 4, CESC was
...
metastatic tumors are distributed across all ten clusters (\it{e.g.} **figure
...
parent pathway (\it{e.g.} ‘Beta-catenin independent WNT signaling’ in ‘Signal
...
highly specialized (\it{e.g.} "PI3K/AKT Signaling in Cancer"). Additionally, for
most of the excluded pathways, a neutral version pathway of the pathway exists
(\it{e.g.} "PIP3 activates Akt signaling"). Finally, we mapped the 18,577
...
approach was to determine the status of each gene (\it{i.e.} expressed or not
