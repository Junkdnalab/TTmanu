...
**supplementary figure S5**). Furthermore, cluster 1 is distinguished
from clusters 2-10 by general lack of P53 mutations (compare **Figure 1f** with the
_TP53_ panel of **Supplementary Figure S5**).
...
Next, we investigated the proportion of samples per cancer type for the significant genes
within a cluster (**Figure 4**; heatmap). Surprisingly, cluster-specific tumors were not 
predominated by one or more highly mutated genes across all cancers. Instead, several 
cancer-specific tumors in these classes had mutation profiles that differ from one another 
( _e.g._ In cluster 2, LIHC was enriched for _PSME4_, KIRC was enriched for _UBC_, and 
PAAD was enriched for _PSMA1_). Taken together, our data identifies a framework cancer 
type specific mutation associated with a specific cluster.  
