...
**Figure 2: Dimensionality reduction of tumor samples using pathway aberrations data 
identifies ten well-defined clusters.** Different perspectives of the same data in 
three-dimensional space are shown using UMAP. Each dot corresponds to a tumor sample. 
Colors indicate the tumor's cluster identity. 
...
a) Sankey diagram (flow is left to right) of the tumor type make up in a cluster. Cluster 
membership is on the left. Cancer type is on the right. b) Projection of breast cancer
subtypes onto the UMAP. c) Projection of BRCA1/2 somatic mutation onto the UMAP. d) 
Projection of tumor stage onto the UMAP, regardless of cancer type. e) Projection of
...
a) Heatmap shows the relative enrichment of each pathway (columns) within each
numbered cluster (rows). Effect size is displayed as colors (orange, enrichment;
purple, depletion). b) Proportion of mutated samples in the each significant
...
We wish to thank David Van Valen, Simon Knott and Kate Lawrenson for critical
