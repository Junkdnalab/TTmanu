...
(GDC)](gdc.cancer.gov) portal. TCGA data were most appropriate for our
study given the relative completeness of the patient metadata, particularly for
survival and staging. We chose to analyze somatic mutations in exome
sequencing data because the affected target gene is known unambiguously.
Therefore, we selected all missense, nonsense, frameshift, stop-gain, untranslated
...
To avoid bias toward larger
pathways ( _i.e._ pathways with more member genes), we counted pathways as disrupted if one or more member genes were mutated. We do not attempt to calculate enrichment for mutations within a pathway, for example. Data
...
(neomorphic or hypermorphic) in genes, and these can in turn act functionally as either activators or repressors.
Within the scope of this study we accept this uncertainty and assume mutations
are generically disruptive to pathways regardless of whether they are
...
Highly expressed genes could also have high mutation rates owing to
transcription induced mutagenesis [@parkGenomicEvidenceElevated2012]. We felt
that such mechanisms result in cell-type-specific biases that might be
...
containing useful information. (**figures 2A, B**). Then we performed a UMAP
...
notable for its lobed structure, with several reproducible projections
regardless of random seed setting. A representative version of this
3D UMAP is shown in **figure 2C**, rotated to enhance the
...
by probability of membership of ≥ 5% in at least one class. Thus, we
...
function (**figure 2D**). HDBSCAN with these settings resulted in ten distinct
high-density clusters which we then projected onto the UMAP (**figure 2E**).
...
the set and then lets the _k_ most similar tumors "vote" as to the identity of
the query tumor sample based on their classification labels. We set _k_ to be
the square root of the number of tumor samples (87). Using this method, we
assigned cluster membership to the remaining tumors (**figure 2F**).
...
cancer types had tumors in every class (see **figure 3A**
...
pathology largely independent of tissue-of-origin. As an
example of one type of cancer that does have a biased pathway profile,
pancreatic adenocarcinoma (PAAD) was predominantly found in class 8, which is
distinguished  by KRAS mutations among others (**supplementary figure S5**). 
But even PAAD comprises tumors from the nine remaining classes, meaning that 
patients suffering these tumors have potentially different underlying molecular
pathologies.
...
pathway disruptions. To illustrate this, we projected annotations for each of the breast
...
pathway for these subtype annotations (**Figure 3B** and **supplemental
...
exclusive. We interpret these data in aggregate to mean
...
factors such as genome stability generally, proof-reading and DNA damage repair, and
...
Tumor staging is based on physico-pathological criteria, including tumor diameter,
...
structures on a common backbone of tumor samples (**figure 2C**) could in
...
has nearly every pathway disrupted (**figure 4**). However, outside of class 10 we don't observe
an obvious trend in the overall mutation burden across the backbone of the UMAP. 
Nonetheless, to test the hypothesis that the molecular-pathway disruption
...
observe any bias among the stages to specific pathway disruption classes
(**figure 3D**), suggesting that stage is not an determining factor in pathway
disruption classes.
...
any of our classes correspond to an especially advanced stage of cancer across
tissue types, we projected the metastasis data onto our UMAP, and surprisingly we 
observed an even distribution of metastases across classes (**figure 3E**). This
...
these analyses reveal the broad importance of many well known pathways that are disrupted
...
mutation frequency (**figure 5A** and **supplementary table ST1**), possibly
owing to the relative diversity of tumors.
...
visualized the enrichment as a heatmap (**figure 5B**). Using this approach, we
...
terms of pathway enrichment. To explore the specific pathways marking
...
**figure 5C** and **supplementary figure S4**). Cluster 2 had the highest pathway
enrichment levels of the three, having mutations in hedgehog signaling,
"β-catenin degradation", "cellular response to hypoxia", "regulation of
cell cycle" and "apoptosis" among others.

### A combination of common driver mutations and tissue-specific genes contribute to common class signatures

Prior efforts to extract common signatures from pan-cancer 
...
thresholds), or a combination of both? To answer this question, we compared top pathway genes for each cluster 
relative to the TCGA background to find differentially mutated genes. We ranked odds ratios
and selected the top ten enriched and depleted genes (pvalue < 0.01) for each cluster 
(**Figure 6**; odds ratios plot). Clusters 7 and 8, which shared multiple enrichment in 
...
that tumors in this cluster could be derivative of either cluster 7 or 8. Clusters 3 and 
...
We also observed that genes that were enriched for one cluster are depleted from others ( _e.g._ 
...
cancer type for the significant genes within a cluster (**Figure 6**; heatmap). Surprisingly, 
...
pathways (**figure 5B**) we applied this method to metastatic vs non-metastatic
tumors, as we find that metastatic tumors are distributed across all ten clusters
( _e.g._ **figure 3E**). Using all non-metastatic tumors as background, we found
very low levels of enrichment (≤ 10%) in a handful of pathways. We
...
found a total of 31 enriched pathways across all clusters (**table 1**). A number of
...
samples were enriched for “Fc epsilon receptor (FCERI) signaling”, a key neutrophil pathway, 
which is also specific to clusters 2, 7 and 8 non-metastatic tumors. Thus, pathway lesions 
...
fully reproducible manuscript in R markdown format. R markdown files and all other
...
Reactome (https://reactome.org/), a knowledge-based pathway
database. The mapping files of ENSEMBL genes to pathways, pathway hierarchy relationships,
and complexes to top pathways were downloaded from https://reactome.org/download-data. Using these data, we imposed pathway criteria to
define basic cellular processes and biochemical pathways: (1) human-derived pathways
...
a neutral version pathway of the pathway exists ( _e.g._ "PIP3 activates Akt signaling"). 
...
differences thus obtained excludes 0 and the mean
credible difference was greater than or equal to 30% enrichment, which
...
The authors have nothing to declare. This work was supported
