...
oncoprint_style <- function(filtered_class, n_path, enrich_cutoff = 0.30) {
  ...
  pheatmap_res <- pheatmap(filtered_mut_data[gene_order,],
         show_rownames = T,
         show_colnames = F,
         treeheight_row = 0,
         treeheight_col = 0,
  ...
  for(p in toppath){
    ...
    if(p != "AUF1 (hnRNP D0) binds and destabilizes mRNA" & p != "Signaling by Type 1 Insulin-like Growth Factor 1 Receptor (IGF1R)") { ## if it is not AUF1 do it normally
  ...
  plot <- plot_grid(l_anno_ggplot[[1]], l_anno_ggplot[[2]], l_anno_ggplot[[3]], l_anno_ggplot[[4]], l_anno_ggplot[[5]], l_anno_ggplot[[6]], l_anno_ggplot[[7]], l_anno_ggplot[[8]], l_anno_ggplot[[9]], l_anno_ggplot[[10]], heatmap_ggplot, align = "h", ncol = 11, axis = "tb", rel_widths = c(rep(0.1, 10), 15))
...
test1 <- oncoprint_style(filtered_class = c(6,7), n_path = 10)
test12 <- oncoprint_style(filtered_class = 12, n_path = 10)
test11 <- oncoprint_style(filtered_class = 11, n_path = 10)
