...
umap_3d_coors <- left_join(umap_3d_coors, tumor_samples, by = "sample_id")
...
fig1D <- ggplot(score_df, aes(x = minPts, y = score)) +
  geom_point(data = sample_df, color = "grey", aes(y = value)) +
  geom_point() + 
  geom_line() +
...
## umap hdb_scan
fig1E <- ggplot(subset(umap_cancertype, clust !=0), aes(x = plot_x, y = plot_y, color = as.factor(clust))) +
  geom_point(data = umap_cancertype, aes(x = plot_x, y = plot_y), colour = "grey", size = 0.25) +
...
## umap knn_clust 
fig1F <- ggplot(subset(umap_cancertype), aes(x = plot_x, y = plot_y, color = as.factor(knn_clust))) +
  geom_point(size = 0.25) + scale_color_manual(values = kelly.colours[-c(1:2)]) +
  guides(color = F) +
  theme_classic() +
  xlab("UMAP 1") +
...
for(n in 1:max(as.numeric(as.character(umap_cancertype$knn_clust)))) { ## loop through all the clusters
  ...
  mut_order[[n]] <- umap_cancertype %>% filter(knn_clust == n) %>%  ## filter for specific class
    dplyr::select(sample_id, plot_x, plot_y, knn_clust, mut_count) %>% ## select relevant columns
...
xaxis_breaks <- mut_order[rownames(mut_order) %in% xaxis_breaks$sample_id,] ## find the tumor samples in these positions
...
mut_order <- mut_order[,c("V1", "knn_clust", "log10mut")] ## selecting relevant column

begin_NA <- data.frame(V1 = paste0("sample", 1:50), 
                       knn_clust = min(as.numeric(mut_order$knn_clust)), ## put lowest cluster to prevent a new facet
                       log10mut = NA) ## put NA values so they don't get plotted
end_NA <- data.frame(V1 = paste0("sample", 51:100), ## repeat the step for the ends
                       knn_clust = max(as.numeric(mut_order$knn_clust)),
                       log10mut = NA)
mut_order <- rbind(begin_NA, mut_order, end_NA) ## rbind the data in a specfic order

mut_order$V1 <- factor(mut_order$V1, levels = mut_order$V1) ## factor samples
mut_order$knn_clust <- factor(mut_order$knn_clust, levels = c(1:12)) ## factor clusters
#### test ####

fig1G <- ggplot(mut_order, aes(x = sample_id, y = log10mut, color = knn_clust)) + 
  geom_point(size = 0.25) + 
  ...
  scale_x_discrete(breaks = c(as.character(mut_order$V1[1]), as.character(xaxis_breaks$V1)), labels = c("n =" ,total_n$Freq)) +
  theme(axis.text.x = element_text(size = 10),
        axis.text.y = element_text(size = 10),
        axis.title.x = element_text(size = 12),
        axis.title.y = element_text(size = 12),
...
## umap by cancer type
fig1H <- ggplot(umap_cancertype[umap_cancertype$project_code %in% c("COAD", "BRCA", "HNSC", "PAAD"),], aes(x = plot_x, y = plot_y, colour = project_code)) +
  geom_point(data = umap_cancertype, aes(x = plot_x, y = plot_y), colour = "grey", size = 0.25) +
  geom_point(size = 0.25) + 
  theme_classic() +
...
fig1I <- ggplot(umap_cancertype %>% filter(project_code == "BRCA" & !is.na(Subtype_Selected)), aes(x = plot_x, y = plot_y, colour = Subtype_Selected)) +
  geom_point(data = umap_cancertype, aes(x = plot_x, y = plot_y), colour = "grey", size = 0.25) +
  geom_point(size = 0.25) +
  theme_classic() +
  ylab("UMAP 2") +
...
fig1J <- ggplot(brca %>% filter(mut_type != "NA"), aes(x = plot_x, y = plot_y, colour = mut_type)) +
  geom_point(data = brca, aes(x = plot_x, y = plot_y), colour = "grey", size = 0.25) +
  geom_point(size = 0.25) + 
  theme_classic() +
  ylab("UMAP 2") +
  xlab("UMAP 1") +
...
## umap stage
fig1K <- ggplot(subset(umap_cancertype, ajcc_pathologic_tumor_stage != "Not Available"), aes(x = plot_x, y = plot_y, colour = ajcc_pathologic_tumor_stage)) +
  geom_point(data = umap_cancertype, aes(x = plot_x, y = plot_y), colour = "grey", size = 0.25) +
  geom_point(size = 0.25) + 
  theme_classic() +
  ylab("UMAP 2") +
...
## umap metastasis
fig1L <- ggplot(umap_cancertype, aes(x = plot_x, y = plot_y)) +
  geom_point(data = umap_cancertype, aes(x = plot_x, y = plot_y), colour = "grey", size = 0.25) +
  geom_point(data = subset(umap_cancertype, ajcc_metastasis_pathologic_pm == "M0"), aes(x = plot_x, y = plot_y, colour = ajcc_metastasis_pathologic_pm), size = 0.25) + 
  geom_point(data = subset(umap_cancertype, ajcc_metastasis_pathologic_pm == "M1"), aes(x = plot_x, y = plot_y, colour = ajcc_metastasis_pathologic_pm), size = 0.25) + 
  ylab("UMAP 2") +
  xlab("UMAP 1") +
  coord_fixed() +
  labs(tag = "L", color = "") +
...
fig1A + fig1B + fig1C + fig1D + fig1E + fig1F + fig1G + fig1H + fig1I + fig1J + fig1K + fig1L + plot_layout(design = layout)
