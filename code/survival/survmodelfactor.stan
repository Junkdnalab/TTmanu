...
model{
  ...
  for(i in 1:Np){
    ...
    int cl = tclass[i];
    if(eventtype[i] == 1){
      target += ourmodel_lpdf(tevent[i]| age[i],agerate[gender[i]] * 0.05 ,ktis[tis]*kcls[cl],r20[gender[i]] * 1e-5);
    }else{
      target += ourmodel_lccdf(tevent[i]| age[i],agerate[gender[i]] * 0.05 ,ktis[tis]*kcls[cl],r20[gender[i]] * 1e-5);
    }
  }
}
