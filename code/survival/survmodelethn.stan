...
// we only really have usable data on white and black races, so
// we use race = 1 for white, and race = 2 for black and split out 
...
parameters{
  ...
  real<lower=0.0> kw[Nt,Nc];
  real<lower=0.0> ktisw[Nt];
  real<lower=0.0> kb[Nt,Nc];
  real<lower=0.0> ktisb[Nt];  
...
model{
  ...
  for(tis in 1:Nt){
    ktisw[tis] ~ gamma(1.2,0.2/1.0);
    ktisb[tis] ~ gamma(1.2,0.2/1.0);
    for( cl in 1:Nc){
      kw[tis,cl] ~ gamma(3.5,2.5/1.0);
      kb[tis,cl] ~ gamma(3.5,2.5/1.0);
    }
  }
  ...
  for(i in 1:Np){
    ...
    if(race[i] == 2){
      if(eventtype[i] == 1){
	target += ourmodel_lpdf(tevent[i]| age[i],ageratew[gender[i]] * 0.05 ,ktisw[tis]*kw[tis,cl],r20w[gender[i]] * 1e-5);
      }else{
	target += ourmodel_lccdf(tevent[i]| age[i],ageratew[gender[i]] * 0.05 ,ktisw[tis]*kw[tis,cl],r20w[gender[i]] * 1e-5);
      }
    }else{
      if(eventtype[i] == 1){
	target += ourmodel_lpdf(tevent[i]| age[i],agerateb[gender[i]] * 0.05 ,ktisb[tis]*kb[tis,cl],r20b[gender[i]] * 1e-5);
      }else{
	target += ourmodel_lccdf(tevent[i]| age[i],agerateb[gender[i]] * 0.05 ,ktisb[tis]*kb[tis,cl],r20b[gender[i]] * 1e-5);
      }      
    }
  }
}
