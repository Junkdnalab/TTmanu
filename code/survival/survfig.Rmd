```{r, survfig, echo=FALSE, warning=FALSE, error=TRUE,message=FALSE,fig.height=12, fig.width=9}
...
BE_legend <- get_legend(thca[[1]] + theme(legend.direction = "horizontal"))
kmlegend <- theme(legend.direction = "horizontal")


  # ((p1+mytheme)/((pstad[[1]]+mytheme + kmlegend)+(pgbm[[1]]+mytheme + kmlegend) +
  #                  (ppaad[[1]]+mytheme + kmlegend) + (thca[[1]] + mytheme) + (kmlegend) + (pcanc+mytheme) + guide_area() + plot_layout(design = kmlayout, guides = "collect"))) + plot_annotation(tag_levels = "A")
layout = "
AAAA
AAAA
AAAA
...
(p1+mytheme) + (pstad[[1]]+mytheme) + (pgbm[[1]]+mytheme + turn.off.legend) + (ppaad[[1]]+mytheme + turn.off.legend) + (thca[[1]] + mytheme + turn.off.legend) + (pcanc+mytheme) + plot_annotation(tag_levels = "A") + plot_layout(design = layout) 
