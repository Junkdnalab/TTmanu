...
oncoprint_style <- function(filtered_class, n_path, enrich_cutoff = 0.30) {
  class_spec <- tumor_clusters %>% dplyr::select(sample_id, clust_knn) %>% filter(clust_knn %in% filtered_class) %>% .[,.$sample_id] ## sample_id for class specific
  ...
  proteasome_r_anno[grep(pattern = "PSM|SEM|UBE2|UBB|UBC", x = proteasome_r_anno$gene), "psm_gene"] <- 1

  r_anno_color <- c("0" = "white", "1" = "black")
  proteasome_r_anno$gene <- factor(proteasome_r_anno$gene, levels = c(rev(gene_order)))
  ...
  plotlist <- c(l_anno_ggplot, list(heatmap_ggplot), list(r_anno)) ## make a list of plots for cowplot

  plot <- plot_grid(plotlist = plotlist, align = "h", ncol = 12, axis = "tb", rel_widths = c(rep(0.1, length(l_anno_ggplot)), 5, 0.1))
  plot_legend <- plot_grid(plotlist = path_legend, ncol = 1, align = "v")

return(plot_grid(plot, plot_grid(NULL, plot_legend, NULL, ncol = 1, rel_heights = c(1, 1, 1)), ncol = 2, rel_widths = c(2.5, 2), axis = "tbl"))
}

class_spec_fig4 <- oncoprint_style(filtered_class = 2, n_path = 10)
save(class_spec_fig4, file = "../data/class_spec_fig4.Rda")
rm(list = ls(all.names = T))
gc()
```

```{r figure-4-plot, fig.width = 9, fig.height=8}
#out.width= '90%'
load("../data/class_spec_fig4.Rda")
class_spec_fig4
```

<p style="FONT-SIZE:10px; COLOR:#000000; LINE-HEIGHT:12px;">**Figure 3: Mutations in proteasomes found in Class 2 affect multiple enriched pathways.** a) Each column is a tumor in cluster 2, each row represents a gene with mutation (red) or not mutated (grey). The genes are organized according to which of the top distinguishing pathways for this class that they effect. Some genes affect multiple pathways, as indicated by the bars at left. The highlighted genes at right are all proteasomal subunits.</p>