...
cancers projected onto the tumors of the UMAP from **Figure 2C of the main text**.
...
genes involved in genome stability projected on the UMAP from **Figure 2C of the main text**.
...
par(mfrow = c(2,3), mai = c(0.1, 0.1, 0.1, 0.1))
par(mar = c(2,2,0,0))
for(n in pathorder[x[,"start1"]:x[,"end1"]]) {
...
for(p in 1:nrow(startingpointlist)-1) {
  data <- startingpointlist[p,]
  s4plot(x = data)
}
...
(column) from **Figure 2C of the main text**.
...
umap_cancertype$clust_knn <- as.character(umap_cancertype$clust_knn)

clusters178 <- umap_cancertype %>% 
       mutate(color = case_when(clust_knn %in% c("1", "7", "8") ~ clust_knn,
                                  TRUE ~ "Other"))

I_outlines <- c("black", "black", "black", "#00000000")
names(I_outlines) <- c("1", "7", "8", "Other")

I_colors <- c("gold2", "gray51", "springgreen4", "#00000011")
names(I_colors) <- c("1", "7", "8", "Other")

load("../data/s5_data.Rda")
genes <- as.character(unique(s5_data$variable))
...
with(clusters178, 
     scatter3D(plot_y, -plot_z, plot_x, 
                                bg = I_colors[color], col = I_outlines[color],
                                pch = 21, cex = 0.8, lwd = 0.2,
                                theta = 0, phi = 65, scale = F,
                                xlab = "", ylab = "UMAP 3", zlab = "UMAP 1",
                                colvar = NULL))
legend("top", inset = c(0.05), legend=names(I_colors[c(1:3)]), col=I_colors[c(1:3)], pch = 20, bty = "n", horiz = T, cex = 1.2)
title(xlab="UMAP 2", line=0)
plot.new()
...
```
**Supplementary Figure S5. Distribution of oncogenes on UMAP clusters.** The left panel shows the identity of 
clusters 1, 7 and 8. The multipanel on the right shows one of the three oncogenes projected onto the tumors of 
the UMAP from **Figure 2C of the main text**.
...
**Supplementary Table 1. Cluster-specific enriched pathways (effect size >= 0.30).**
```{r enrich-pathway-in-clust, fig.width= 9}
## docx friendly
...
![**Supplemental figure S6. Survival fit independent of disease status.** _The red and blue curves show the survival of people in the US by female and male, respectively. The orange and green curves show the model fits._]
