...
ggplot(umap_cancertype, aes(x = x_2d, y = y_2d, colour = project_code)) + 
  geom_point(data = umap_bg, aes(x = x_2d, y = y_2d), colour = "grey", size = 0.25) +
...
for(n in 1:nrow(figs4_layout)) {
  ...
  ggplot_umap <- ggplot(filtered_path, aes(x = x_2d, y = y_2d, colour = as.character(value))) + 
    geom_point(data = umap_bg, aes(x = x_2d, y = y_2d), colour = "grey", size = 0.1) +
    geom_point(size = 0.1) +
    theme_classic() +
    coord_fixed() +
    xlab("UMAP 1") + ylab("UMAP 2") +
