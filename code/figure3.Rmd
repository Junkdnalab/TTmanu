...
tumor_clusters <- tumor_clusters %>% as.data.frame() %>% dplyr::select(sample_id, clust_knn) ## select the relevant columns

## Getting the total number of tumor samples in each cluster
n_tumor_in_clust <- as.data.frame(table(tumor_clusters$clust_knn)) %>% ## table the number of tumors in each cluster
...
for(n in clust_group) {
  print(n)
  sample_id <- tumor_clusters %>% filter(clust_knn == n) %>% ## filter for tumor class
...
global_path_mut <- cluster_path_count %>% rowSums() %>% as.data.frame() %>% ## global pathway mutation count
  .[rep(1:ncol(.), each = max(tumor_clusters$clust_knn))] %>% ## replicate the column 0 times for the 10 tumor classes
...
gc()
save(moderate_enrich_prob, file = "../data/moderate_enrich_prob.Rda")
...
effect_heatmap$cluster <- factor(effect_heatmap$cluster, levels = c(1:max(as.numeric(effect_heatmap$cluster))))
...
```{r group-pathway-enrichment heatmap, fig.width= 5, fig.height= 10, fig.align= 'center', fig.cap="**Figure 3. Pathway-specific enrichment within clusters.** Heatmap shows relative enrichment of each pathway (rows) within each numbered cluster (columns). The annotation at left indicates the Reactome pathway group at the second level (check this)."}
...
fig3_legend <- plot_grid(NULL, fig3_legend, NULL, ncol = 3)

plot_grid(fig3, fig3_legend, ncol = 1, rel_heights = c(15, 1))
```